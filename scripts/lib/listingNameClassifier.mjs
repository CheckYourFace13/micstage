 ...
 * scripts (which cannot import TS) share ONE listing-name / public-display
 * classifier.
 *
 ...
 */
export const EXACT_JUNK_NAME = new Set(
  [
    "open",
    "must",
    "music",
...
/** Directory / tourism / org / article-heading shells (sync with listingQuality.ts). */
...
export const PLATFORM_OR_TICKET_FLUFF =
...
/** Scraped URL / path-fragment / artifact names. */
const PATH_OR_URL =
...
const AGG_GEO_TOKENS = new Set([
  ...
  "greater", "area", "village", "borough", "lake",
]);
 ...
 */
export function suggestCanonicalListingName(name) {
  let n = (name ?? "").trim();
  if (!n) return null;
  const original = n;
