...
export async function POST(request: Request) {
  ...
  const read = await readNamedImageFile(formData, "artistUploadSecondary");
  if ("error" in read) {
    return redirectTo(`${ARTIST_DASHBOARD_HREF}?profileError=uploadMissing`);
  }

  const stored = await storeProfileImage(read.buf, read.type, `artist/${musicianId}/secondary-${Date.now()}`);
  if (!stored.ok) {
  ...
  await requirePrisma().musicianUser.update({
    where: { id: musicianId },
    data: { imageSecondaryUrl: stored.publicUrl },
  });
