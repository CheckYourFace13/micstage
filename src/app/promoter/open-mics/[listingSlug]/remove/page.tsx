...
import { assertPromoterCanRemoveListing } from "@/lib/publicListings/openMicSelfRemoval";
import { buildPublicMetadata } from "@/lib/publicSeo";
import { FormSubmitButton } from "@/components/FormSubmitButton";
import { removeOpenMicAsPromoterAction } from "../../removeActions";
...
export async function generateMetadata(): Promise<Metadata> {
  return buildPublicMetadata({
    ...
    path: "/promoter/open-mics/remove",
    index: false,
  });
}

export default async function RemoveOpenMicConfirmPage(props: {
  ...
  if (!session || session.kind !== "promoter") {
    redirect(`/login/promoter?next=${encodeURIComponent(`/promoter/open-mics/${listingSlug}/remove`)}`);
  }
        ...
        <div className="min-h-dvh bg-black text-white">
          <main className="mx-auto max-w-lg px-4 py-12">
            ...
            <p className="mt-3 text-sm text-white/70">
              This listing isn&apos;t linked to your promoter account. Contact MicStage if you think that&apos;s wrong.
            </p>
            <Link href="/promoter" className="mt-6 inline-block text-[rgb(var(--om-neon))] underline">
              Back to my account
            </Link>
          </main>
        </div>
  ...
  return (
    <div className="min-h-dvh bg-black text-white">
      <main className="mx-auto max-w-lg px-4 py-10 sm:px-6 sm:py-14">
        ...
        <form action={removeOpenMicAsPromoterAction} className="mt-8 grid gap-3">
