...
function popupHtml(v: OpenMicMapVenueDto): string {
  ...
  return `
    <div style="min-width:220px;max-width:280px;font-family:system-ui,sans-serif;font-size:13px;line-height:1.35;color:#f5f5f5">
      ...
      <a href="${escHtml(v.href ?? `/venues/${v.slug}`)}" style="display:inline-block;background:${OPEN_MIC_MAP_NEUTRAL_MARKER_HEX};color:#fff;text-decoration:none;font-weight:600;padding:8px 12px;border-radius:8px;font-size:13px;box-shadow:0 1px 2px rgba(0,0,0,.2)">${v.isPublicListing ? "View listing" : v.href && String(v.href).startsWith("/nights/") ? "OPEN SPOTS · Sign up" : "View venue &amp; book"}</a>
    </div>
  `;
}
