...
import { deleteVenueSlot, moveVenueBookingAction, updateVenueSlotLine } from "@/app/venue/actions";
...
import { minutesToTimeInputValue, minutesToTimeLabel } from "@/lib/time";

type SlotRow = Slot & { booking: Booking | null };

type Props = {
  venueId: string;
  slot: SlotRow;
  /** Other slots on the same night instance — for Move to start time. */
  siblingSlots?: SlotRow[];
...
export function VenueSlotManagementRow({
  ...
  const moveTargets = siblingSlots.filter((s) => s.id !== slot.id);

  return (
    <div className="grid gap-2 border-b border-white/10 py-2.5 last:border-b-0">
      ...
      {active && moveTargets.length > 0 ? (
        <form
          action={async (fd) => go(await moveVenueBookingAction(fd))}
        ...
        >
          ...
          <label className="flex items-center gap-1 text-xs text-white/70">
            ...
            <select
              ...
              required
              defaultValue=""
              className="h-8 rounded-md border border-white/15 bg-black/50 px-2 text-xs text-white"
            >
              <option value="" disabled>
                Start time…
              </option>
              {moveTargets.map((s) => {
                const other = s.booking && !s.booking.cancelledAt ? s.booking : null;
                const label = other
                  ? `${minutesToTimeLabel(s.startMin)} (swap with ${other.performerName})`
          ...
          <FormSubmitButton
            label="Move"
            pendingLabel="…"
            className="h-8 rounded-md border border-white/20 px-3 text-xs font-semibold text-white"
          />
        </form>
      ) : null}
    </div>
