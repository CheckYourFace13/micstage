...
export type SocialPayloadJobRunResult =
...
/** Drain pending SOCIAL_PAYLOAD_RENDER jobs (website email mining). */
export async function runMarketingSocialPayloadBatch(
  prisma: PrismaClient,
  batchSize: number,
): Promise<{
  batchSize: number;
  ...
  return { batchSize, processed: results.length, emailsFound, pendingRemaining, results };
}
