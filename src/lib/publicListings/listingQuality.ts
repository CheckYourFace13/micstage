...
export function classifyListingName(name: string): ListingNameRejection | null {
  ...
  if (DIRECTORY_OR_ORG_SHELL.test(n) || ORG_OR_FORUM_SHELL.test(n) || LIVE_MUSIC_GEO_SHELL.test(n)) {
    return "AGGREGATOR_OR_DIRECTORY";
  }
  if (EDITORIAL.test(n) || EDITORIAL_ROUNDUP.test(n)) return "NON_VENUE_TITLE";
 ...
 * OUTDATED (rejected/stale) and UNVERIFIED (undiscovered) rows, removed rows,
 * and junk-named rows are hidden entirely. NEEDS_REVIEW rows still render
 * (noindexed, absent from every browse surface) so claim-invite recipients can
 * reach them.
 */
export function isPublicListingRenderable(listing: {
  ...
  if (listing.removedAt) return false;
